...
pub struct ServerContextLayer<T: 'static + Send + Sync> {
    extentable_context: Arc<T>,
    internal_context: Arc<ServerContext>,
    ...
    T: 'static + Send + Sync,
{
    ...
    fn layer(&self, service: S) -> Self::Service {
        let mut context: HashMap<TypeId, Arc<dyn Any>> = HashMap::new();
        ...
        ServerContextService { service, context: Arc::new(context) }
    }
}

pub struct ServerContextService<S> {
    service: S,
    context: Arc<HashMap<TypeId, Arc<dyn Any>>>,
}
    ...
    S: Service<Request>,
{
    ...
    type Error = S::Error;
    type Future = TaskLocalFuture<Arc<HashMap<TypeId, Arc<(dyn Any + 'static)>>>, S::Future>;
    ...
    fn call(&mut self, request: Request) -> Self::Future {
        SERVER_CONTEXT.scope(self.context.clone(), {
